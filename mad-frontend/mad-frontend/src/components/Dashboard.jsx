...
const Dashboard = () => {
    ...
    // Fetch Lead Stats for the widget
    useEffect(() => {
        const fetchLeadStats = async () => {
            try {
                const data = await leadService.getLeadsStats();
                setLeadStats(data);
            } catch (err) {
        ...
        <div className="flex h-screen bg-slate-50 font-sans text-slate-800 overflow-hidden">
            ...
            <aside className={`fixed md:static inset-y-0 left-0 bg-white border-r border-slate-200 flex flex-col shadow-2xl md:shadow-none transition-transform duration-300 z-40 w-72 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
                ...
                <nav className="flex-1 p-4 space-y-1 overflow-y-auto custom-scrollbar">
                    ...
                    <NavItem icon={<MessageSquare />} label="Leads Pipeline" onClick={() => navigate('/leads')} />
                ...
                <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-8 custom-scrollbar scroll-smooth">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                        ...
                        <div
                            ...
                            className="bg-gradient-to-br from-violet-600 to-indigo-700 p-6 rounded-2xl shadow-xl shadow-violet-200 text-white relative overflow-hidden group hover:scale-[1.02] transition-transform duration-300 cursor-pointer"
                        >
                            <div className="flex justify-between items-start mb-3 relative z-10">
                                <div>
                                    <p className="text-violet-100 text-xs font-bold uppercase tracking-wider mb-1">Leads Today</p>
                                    <p className="text-4xl font-bold">{leadStats?.todayCount ?? '—'}</p>
                                </div>
                            ...
                            <div className="flex gap-2 flex-wrap relative z-10">
                                <span className="text-[11px] bg-white/15 px-2 py-0.5 rounded-full font-semibold">
                                    {leadStats?.totalNew ?? 0} New
                                </span>
                                <span className="text-[11px] bg-white/15 px-2 py-0.5 rounded-full font-semibold">
                                    {leadStats?.totalContacted ?? 0} Contacted
                                </span>
                                <span className="text-[11px] bg-white/15 px-2 py-0.5 rounded-full font-semibold">
                                    {leadStats?.totalAll ?? 0} Total
                                </span>
                            </div>
